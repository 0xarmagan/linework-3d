#!/bin/bash
# Grain + encode the 360 void variant.  (NAME env var sets output basename)
#
...
ffmpeg -y -loglevel error -framerate $FPS -i "$SRC/f_%04d.png" \
  ...
  -auto-alt-ref 0 -row-mt 1 "$OUT/${NAME}_360.webm"

echo "=== verify ==="
for f in "$OUT/${NAME}_360_black.mp4" "$OUT/${NAME}_360.webm"; do
  printf '%s\n' "$f"
